...
let box = document.querySelector("#content-pokemon");

if (box && typeof pokemons !== 'undefined') {
    pokemons.forEach((poke) => {
        let pokimon = document.createElement("div");
        pokimon.classList.add("poke-box");

        let typesHTML = poke.type
            .map(t => `<button class="btn btn-${t.toLowerCase()}">${t}</button>`)
            .join("");

        pokimon.innerHTML = `
            <p class="poke-id">№ ${poke.id}</p>
            <img class="poke-img" src="${poke.img}" alt="${poke.name}" />
            <h3 class="poke-name">${poke.name}</h3>
            <p class="poke-info"><span>${poke.height}</span> — <span>${poke.weight}</span></p>
    ...
    console.error("Ошибка: Контейнер #content-pokemon не найден или массив pokemons не загружен.");
}